...
This mechanism has been integrated into Bitcore-Node-GDC which allows for notification to be broadcast through Insight API in one of two ways:
* WebSocket: 
* API: 
...
By default, the GUARDIAN daemon will launch using the following constant:

```
static const int DEFAULT_INSTANTX_DEPTH = 5;
```

This value can be overridden by passing the following argument to the GUARDIAN daemon:
