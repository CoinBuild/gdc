...
{
    /* current difficulty formula, gdc - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
	...
	if(pindexLast->nHeight >= 10 && pindexLast->nHeight <= Params().LAST_POW_BLOCK()) {
        uint256 bnNew;
		bnNew = ~uint256(0) >> 20;
